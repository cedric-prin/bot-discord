...
class SpamFilter {
  ...
  async check(message, config) {
    ...
    // La configuration passée en paramètre doit écraser les défauts
    ...
    if (recentMessages.length > config.maxMessages) {
      return {
        ...
        reason: `Flood détecté (${recentMessages.length} messages en ${config.timeWindow / 1000}s)`,
        matchedContent: null
      };
    }
    ...
    const duplicates = recentMessages.filter(
      msg => msg.content === currentContent
    );
    ...
    if (recentMessages.length >= 3) {
      ...
      if (similarCount >= config.maxDuplicates) {
        return {
          triggered: true,
          action: config.action,
